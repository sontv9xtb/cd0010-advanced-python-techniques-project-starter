...
The `query` subcommand searches for close approaches that match given criteria:
    ...
    $ python3 main.py query --start-date
    2020-01-01 --end-date 2020-01-31 --max-distance 0.025
    $ python3 main.py query --start-date
    2050-01-01 --min-distance 0.2 --min-velocity 50
    $ python3 main.py query --date
     2020-03-14 --max-velocity 25 --min-diameter 0.5 --hazardous
    $ python3 main.py query --start-date
    2000-01-01 --max-diameter 0.1 --not-hazardous
...
If needed, the script can load data from data
files other than the default with
`--neofile` or `--cadfile`.
"""
import argparse
import cmd
...
# The current time, for use with the
# kill-on-change feature of the interactive shell.
_START = time.time()
